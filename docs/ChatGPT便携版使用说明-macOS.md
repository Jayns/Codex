...
`ChatGPT Launcher` 已经过 Apple 签名与公证，双击即可正常打开。如果这份压缩包是从网络下载或通过其他方式传输来的，系统可能会弹出“这是从互联网下载的应用程序，你确定要打开它吗？”的确认框，点击 **打开** 即可，之后不会再提示。

> 如果双击后仍然出现“Apple 无法验证”或“已损坏”的拦截提示，说明这份拷贝不是公证版（例如旧版本或传输过程中签名被破坏），处理方法见文末“常见问题”。
...
| 现象 | 处理方式 |
| --- | --- |
| 首次打开提示“Apple 无法验证”或“已损坏，应移到废纸篓” | 正式发布的 `ChatGPT Launcher` 已公证，双击应直接打开，不会出现这类拦截。若确实遇到，说明这份拷贝不是公证版（旧版本，或传输过程中签名被破坏）：先在“终端”运行 `xattr -dr com.apple.quarantine "ChatGPT Launcher.app"` 清除隔离标记后重试；仍不行则手动放行——弹窗里先点“完成”（不要点“移到废纸篓”），再到 系统设置 → 隐私与安全性，在“已阻止……”提示处点“仍要打开”，最后在二次确认框里再点一次“仍要打开”。 |
